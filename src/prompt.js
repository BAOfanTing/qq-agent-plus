...
function speakOrNot(participation) {
  ...
  return [
    ...
    '- 被 @、点名或直接提问时，默认要回一句（可以短、可以敷衍、可以怼回去）；只有明显与你无关、对方在 @ 别人、或纯刷屏误 @ 时才不回。',
...
function qqSceneRules(grounded = false) {
  ...
  const lines = [
    ...
    '- 有人明确冲着你说话（@ 你、问你问题、接你的话茬）时，默认要回一句——可以短、可以敷衍、可以嘴硬，但别已读不回让人干等；只有明显与你无关或误 @ 时才不回。',
  ...
  if (vision) {
    lines.push(
      ...
      '- 图库可以自己攒：看到别人发的图有意思、能当表情用（好笑/欠揍/适合回怼），先 get_message_images 看一眼，确认好玩就用 collect_sticker 存进表情库（顺手写一句备注），以后就能用 send_sticker 发出来；list_stickers 看现有图库。挑真的会用的存，别什么都收。',
