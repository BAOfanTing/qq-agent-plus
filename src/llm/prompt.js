 ...
 * （装傻、随口应付、已读乱回、没人叫也可以主动插话……）。模型默认把靠后的当更具体的要求，
 * 于是"改人设没效果" —— 2026-09-21 线上的服从版角色卡就是这么被压住的。
 * 安全规则仍然最高；工具用法与"发言只能走 send_message"这类机制约束也不受角色设定影响。
 */
function priorityNote() {
  return [
    '【优先级】安全规则 ＞【管理员附加规则】＞【角色设定】＞ 下面的平台默认风格。',
    '- 人格、口吻、称呼、态度、喜恶，以【角色设定】和【管理员附加规则】为准；它们与平台默认风格冲突时，按角色设定来。',
    '- 平台默认风格里那些"允许 / 可以"（装傻、敷衍、反问、已读乱回、没被叫也可以插话……）只是没写角色设定时的默认值，不是必须遵守的规则。',
    '- 但安全规则、工具用法、"发言只能走 send_message"这类机制约束不受角色设定影响。'
  ].join('\n');
}
