...
export function sanitizeUserText(text) {
  ...
  // ⚠️ 这份名单要与 prompt.js 的段头同步（新加段头时把它也加进来）：
  //    test/prompt-safety.test.mjs 会从 prompt.js 抽出所有【…】段头，逐个断言能被弱化。
  s = s.replace(
    /[\[【［][\s\u200b\u200c\u200d]*((?:本次唤醒|系统提醒|系统|管理员(?:附加规则)?|owner|角色扮演|会话令牌|当前时间|过去状态|此刻状态|记忆(?:与会话交接)?|上次会话交接|上次生命周期检查点|当前对话线程|优先级|角色设定|安全规则|工作方式|反\s*AI\s*味|保持主体性|该说\s*\/\s*不该说|群聊不是客服队列|像真人一样|不要当群管家|引用与点名|可用表情包|QQ\s*场景规则|发送与汇报禁令|发言与沉默|发言的唯一通道|分条发送|看图先读情绪|自然交流与可靠边界|每次运行的决策顺序|生命周期续接|空格不是分句符号)[^\]】］]*)[\]】］]/gi,
    '（$1）'
  );
  return s;
}
