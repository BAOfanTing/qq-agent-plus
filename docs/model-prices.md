...
**从配置里删掉一个渠道就会当场撤掉它的价**（2026-09-21 修）：启动、以及每次保存配置都会
重新按 `channelPriceFeeds` 对齐一次查价层 —— 手工编辑 `config.json` 删掉渠道也一样，
不用重启（以前那张表会一直留在内存里按老价算钱）。
...
| 我按渠道价：官方价 × 折扣 | `costMode: 'multiplier'` + `costMultiplier` | 表里的价统一乘折扣，标成**实付**口径（中转站用户通常只知道一个折扣）；填 `0` = 这个渠道不花钱 |
| 我按月付 ¥X/月 | `costMode: 'subscription'` + `costMonthlyFee` | 所有模型按固定月支出显示，不再按 token 算（订阅制、本地自建） |

用户手填的任何价（`modelPrices`）都优先于账户口径。

**0 是合法值，不是"没填"**（2026-09-21 修）：

- `modelPrices` 里写 `{ "in": 0, "out": 0 }` = 明确声明的免费价（赠送额度、本地模型），
  不会回落到价格表；想表达"不计费"也可以写 `billing: "none"`（更明确，用量页标「本地」）。
  判据是"写没写 `in`/`out` 字段"，不是"算出来是不是 0"。
- `costMultiplier: 0` = 这个渠道不花钱（价格 ×0）；没填才是 1 倍。
- 空条目（`{}` 或只写了 `note`）仍然算"没填"，照常回落到价格表。
...
**并发与来源标记**（2026-09-21 修）：同一组地址的刷新会合并成一次请求（手动「立即拉取」
撞上定时刷新时不再互相覆盖）；换了地址还没拉成功时，状态里带 `sourceStale: true`，
设置页会说明"当前生效的仍是上一次成功拉取的旧地址"，不会拿新地址假装已生效。
...
`from` / `until` 要写完整 ISO 时间（含时区）。**写错的边界会让整条别名不生效**
（fall back 到模型自己的价），而不是被当成"没写边界" —— 后者会让 `until` 写错的别名
永不失效、`from` 写错的别名回溯到全部历史，都是在拿错价算钱（`aliasTargetAt`，2026-09-21 修）。
